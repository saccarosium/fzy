...
{
	c->strings =
		safe_realloc(c->strings, new_capacity * sizeof(const char *));
...
static void worker_get_next_batch(struct search_job *job, size_t *start,
				  size_t *end)
{
...
static void *choices_search_worker(void *data)
{
	...
	qsort(result->list, result->size, sizeof(struct scored_result),
	      cmpchoice);
	...
	for (unsigned int step = 0;; step++) {
		...
		if ((errno = pthread_join(job->workers[next_worker].thread_id,
					  NULL))) {
			perror("pthread_join");
			exit(EXIT_FAILURE);
		}
